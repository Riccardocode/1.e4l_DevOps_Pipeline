...
for description in "production-backend-shell" "production-frontend-shell"; do
...
sudo gitlab-runner register \
  ...
  --description "production-backend-shell" \
  --tag-list "production-vm-backend-shell" \
  --executor "shell" \
  --run-untagged="true"
...
sudo gitlab-runner register \
  ...
  --url "http://192.168.56.9/gitlab/" \
  --registration-token "$token" \
  --description "production-frontend-shell" \
  --tag-list "production-vm-frontend-shell" \
  --executor "shell" \
  --run-untagged="true"
