...
docker cp "$APP_SOURCE_PATH/." $CONTAINER_NAME:/app || { echo "Error copying source code. Exiting."; exit 1; }
...
mkdir -p "$BACKEND_REPO_PATH/build/libs"
chmod -R 755 "$BACKEND_REPO_PATH/build"

# Step 1.6 - Copy the built JAR file to the host
echo "Copying JAR file to host..."
docker cp "$CONTAINER_NAME:/app/build/libs/." "$BACKEND_REPO_PATH/build/libs" || { echo "Error copying JAR file to host. Exiting."; exit 1; }
...
docker build -t lu.uni.e4l.platform.backend.dev:rc "$BACKEND_REPO_PATH" || { echo "Error during Docker image build. Exiting."; exit 1; }
